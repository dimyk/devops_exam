terraform {
  ...
  # Збереження tfstate файлу у хмарі (як вимагається в завданні)
  backend "s3" {
...
resource "digitalocean_vpc" "mykhalchuk_vpc" {
  name     = "mykhalchuk-vpc"
  region   = "fra1"           # Найближчий до України регіон [cite: 7]
  ip_range = "10.10.10.0/24"  # [cite: 8]
}
...
resource "digitalocean_droplet" "mykhalchuk_node" {
  ...
  region   = "fra1"             # Ідентичний до VPC [cite: 17]
  size     = "s-2vcpu-4gb"      # Відповідає вимогам Minikube/Kubernetes [cite: 15]
...
resource "digitalocean_spaces_bucket" "mykhalchuk_bucket" {
  ...
  region = "fra1" # Ідентичний до VPC [cite: 21]
}terraform {
  required_providers {
    digitalocean = {
      source  = "digitalocean/digitalocean"
